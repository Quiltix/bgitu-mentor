...
@Tag(
    name = "Applications",
    description = "Управление заявками на менторство между студентами и менторами")
public class ApplicationController {
  ...
  @Operation(
      summary = "Отправить заявку на менторство",
      description = "Позволяет студенту отправить заявку выбранному ментору")
  public ResponseEntity<ApplicationDetailsResponseDto> requestMentorship(
      Authentication authentication, @RequestBody @Valid ApplicationCreateRequestDto dto) {
  ...
  @Operation(
      summary = "Ответить на заявку",
      description = "Ментор обновляет статус существующей заявки (Принять/Отклонить)")
  ...
  public List<?> getMyApplications(
    ...
    Role userRole =
        authentication.getAuthorities().stream()
            ...
            .map(
                roleString -> roleString.startsWith("ROLE_") ? roleString.substring(5) : roleString)
            ...
            .orElseThrow(
                () -> new IllegalStateException("Не удалось определить роль пользователя."));
