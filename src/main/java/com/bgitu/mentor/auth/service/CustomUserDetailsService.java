...
public class CustomUserDetailsService implements UserDetailsService {
  ...
  /**
   * Загружает данные пользователя по его уникальному имени (в данном случае, email). Этот метод
   * вызывается Spring Security AuthenticationManager во время процесса аутентификации по логину и
   * паролю.
   *
    ...
    if (user instanceof Mentor) {
      ...
      throw new IllegalStateException(
          "Для пользователя с id="
              + user.getId()
              + " не определена конкретная роль (Ментор/Студент).");
    }
